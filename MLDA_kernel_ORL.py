...
################
 ...
 Ac2[ii] = accuracy2
 #if ii<3:
 # axs[0, ii].plot(Ac[1:], label="kernel MLDA")
 # axs[0, ii].plot(Ac2[1:], label="MLDA")
 # axs[0, ii].set_title("ORL data set %i" % ii)
 # axs[0, ii].set_xlabel("Feature Dimension")
 # axs[0, ii].set_ylabel("Accuracy rate (%)")
 # axs[0, ii].set_ylim([60, 100])
 # axs[0, ii].legend()
 # fig.tight_layout()
 # fig.suptitle('ORL Database Polynomail kernel degree 3 regularization $\mu = 10^{-3}$', fontweight="bold")
 #else:
 # axs[1, iik].plot(Ac[1:], label="kernel MLDA")
 # axs[1, iik].plot(Ac2[1:], label="MLDA")
 # axs[1, iik].set_title("ORL data set %i" % ii)
 # axs[1, iik].set_xlabel("Feature Dimension")
 # axs[1, iik].set_ylabel("Accuracy rate (%)")
 # axs[1, iik].set_ylim([60, 100])
 # axs[1, iik].legend()
 # fig.tight_layout()
 # fig.suptitle('ORL Database Polynomail kernel degree 3 regularization $\mu = 10^{-3}$', fontweight="bold")
 # iik+=1
